  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(STORES.TRAINING_DATA)) {
        ...
        trainingStore.createIndex('question', 'question', { unique: false });
      }
      
      // Calculation patterns store
      if (!db.objectStoreNames.contains(STORES.CALCULATION_PATTERNS)) {
        const calcStore = db.createObjectStore(STORES.CALCULATION_PATTERNS, { keyPath: 'id', autoIncrement: true });
        calcStore.createIndex('pattern', 'pattern', { unique: false });
      }
      
      // Default training data store
      if (!db.objectStoreNames.contains(STORES.DEFAULT_TRAINING)) {
        db.createObjectStore(STORES.DEFAULT_TRAINING, { keyPath: 'id', autoIncrement: true });
      }
      
      // Uploaded files metadata store
      if (!db.objectStoreNames.contains(STORES.UPLOADED_FILES)) {
        const filesStore = db.createObjectStore(STORES.UPLOADED_FILES, { keyPath: 'filename' });
...
async function getResponse(input) {
  ...
  // Step 1: Check if input contains numbers (potential calculation)
  const containsNumbers = /\d/.test(sanitizedInput);
  
  // Step 2: Check if input matches trained function patterns
  if (containsNumbers) {
  ...
  // Step 3: Check for exact match in training data
  const exactMatch = await findExactMatch(sanitizedInput);
  if (exactMatch) {
    return exactMatch.answer;
  }
  
  // Step 4: Word-by-Word Mode
  ...
  for (const word of words) {
    if (/^\d+$/.test(word)) {
      wordResults.push(`${word}: number`);
      hasNumber = true;
    } else {
      const wordMatch = await findExactMatch(word);
      if (wordMatch) {
        wordResults.push(`${word}: ${wordMatch.answer}`);
      } else {
        wordResults.push(`${word}: not trained yet`);
      }
    }
  }
  ...
  // Step 5: Fuzzy matching for suggestions
  const similarMatches = await findSimilarMatches(sanitizedInput, 0.6);
  if (similarMatches.length > 0) {
    return `Did you mean "${similarMatches[0].question}"? I know the answer to that.`;
  }
  
  // Step 6: No match
  return "I'm still learning this. Can you rephrase?";
}
...
function calculateSimilarity(str1, str2) {
  ...
  // Fallback to Levenshtein distance for short strings or no word matches
  const matrix = [];
...
async function sendMessage() {
  ...
  try {
    ...
    document.getElementById('chat-box').removeChild(loadingElement);
